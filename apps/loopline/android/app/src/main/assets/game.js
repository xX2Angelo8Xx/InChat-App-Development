...
(() => {
  ...
  function renderSum(){
    ...
    p.tiles.forEach((v,i)=>{const c=document.createElement('button');const rowDone=rowSum(p,Math.floor(i/3))===p.rows[Math.floor(i/3)],colDone=colSum(p,i%3)===p.cols[i%3];c.className='cell'+(p.selected===i?' selected':'')+(rowDone?' row-done':'')+(colDone?' col-done':'');c.textContent=v;c.setAttribute('aria-label','Feld '+(i+1)+', Zahl '+v);c.onclick=()=>{if(busy)return;if(p.selected===null){p.selected=i;renderSum();return;}if(p.selected===i){p.selected=null;renderSum();return;}[p.tiles[p.selected],p.tiles[i]]=[p.tiles[i],p.tiles[p.selected]];p.selected=null;stats.swaps++;beep(440+solvedCount(p)*30);renderSum();if(solvedCount(p)===6)finish();};b.append(c);});
  ...
  function reset(){if(!puzzle||busy)return;run++;stats.resets++;if(puzzle.type==='path'){puzzle.trace=[];renderPath();}else if(puzzle.type==='sum'){puzzle.tiles=puzzle.initial.slice();puzzle.selected=null;stats.swaps=0;renderSum();}else{puzzle.answer=0;puzzle.phase='ready';renderMemory();$('feedback').textContent='Bereit? Starte die Folge.';}}
  function hint(){
    ...
    else if(p.type==='sum'){const i=p.tiles.findIndex((v,x)=>v!==p.solution[x]);if(i<0)return;const j=p.tiles.indexOf(p.solution[i]);stats.hints++;const cells=$('play-area').querySelectorAll('.cell');cells[i].classList.add('hint-cell');cells[j].classList.add('hint-cell');setTimeout(()=>{cells[i].classList.remove('hint-cell');cells[j].classList.remove('hint-cell');},2400);$('feedback').textContent='Tausche die beiden markierten Zahlen.';}
