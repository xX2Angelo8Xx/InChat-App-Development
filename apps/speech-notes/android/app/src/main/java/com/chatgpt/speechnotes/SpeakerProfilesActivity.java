...
public final class SpeakerProfilesActivity extends Activity {
    ...
    private void showProfiles() {
        ...
        TextView title = label("Sprecherprofile", 28, true); root.addView(title);
        ...
        root.addView(space(18));
        enroll = new Button(this); enroll.setAllCaps(false); enroll.setText("+ Sprecherprofil aufnehmen"); enroll.setOnClickListener(v -> startEnrollment()); root.addView(enroll);
    ...
    private void startEnrollment() {
        ...
        root.addView(space(18));
        text = label(PAGE1, 20, false); text.setLineSpacing(0f, 1.25f); root.addView(text);
    ...
    private void recordAndEncode() {
        ...
        try {
            ...
            short[] all = new short[SR * RECORD_SECONDS]; short[] buf = new short[2048]; int pos = 0;
            rec.startRecording(); long start = SystemClock.elapsedRealtime();
            while (pos < all.length) {
                int n = rec.read(buf, 0, Math.min(buf.length, all.length - pos));
                if (n < 0) throw new IllegalStateException("AudioRecord Fehler " + n);
                if (n == 0) continue;
                System.arraycopy(buf, 0, all, pos, n); pos += n;
                long elapsed = SystemClock.elapsedRealtime() - start;
                if (elapsed >= 25000 && elapsed < 25500) runOnUiThread(() -> { status.setText("Seite 2 von 2 · weiter sprechen"); text.setText(PAGE2); });
                final int sec = Math.min(RECORD_SECONDS, (int)(elapsed / 1000));
                if ((sec % 5) == 0) runOnUiThread(() -> status.setText((sec < 25 ? "Seite 1 von 2" : "Seite 2 von 2") + " · " + sec + "/50 s"));
            }
            rec.stop(); rec.release(); rec = null;
            runOnUiThread(() -> status.setText("Profil wird lokal berechnet …"));
            ArrayList<float[]> embeddings = new ArrayList<>();
            try (SpeakerEmbeddingModel model = new SpeakerEmbeddingModel(this)) {
                int win = SR * 3, hop = SR * 3;
                for (int s = 0; s + win <= all.length; s += hop) {
                    short[] chunk = new short[win]; System.arraycopy(all, s, chunk, 0, win);
                    if (rms(chunk) < 0.012) continue;
                    embeddings.add(model.embed(chunk));
                }
            }
    ...
    private void askName(List<float[]> embeddings) {
        ...
        new AlertDialog.Builder(this).setTitle("Profil erfolgreich aufgenommen").setMessage("Wie heißt diese Person?")
                .setView(input).setCancelable(false)
                .setPositiveButton("Speichern", (d,w) -> {
                    try { new SpeakerProfileStore(this).create(input.getText().toString(), embeddings); enrolling = false; showProfiles(); }
                    catch (Throwable t) { enrolling = false; Toast.makeText(this, t.getMessage(), Toast.LENGTH_LONG).show(); showProfiles(); }
